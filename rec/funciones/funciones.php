<?php
	...
	function isNull($id,$nombre,$apellido,$telefono,$direccion, $usuario,$email, $password, $con_password){
		if(strlen(trim($id)) < 1 || strlen(trim($nombre)) < 1 || strlen(trim($apellido)) < 1 || strlen(trim($telefono)) < 1 || strlen(trim($direccion)) < 1 || strlen(trim($usuario))< 1 || strlen(trim($email))< 1 || strlen(trim($password))< 1 || strlen(trim($con_password))< 1 )
		{
			return true;
			} else {
			return false;
	...
	function usuarioExiste($nombre_user)
	{
		global $mysqli;
